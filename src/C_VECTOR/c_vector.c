...
int c_vector_push_back(c_vector* cVector, void* data, size_t offset, size_t count) {
	...
	memcpy((unsigned char*)(cVector->items) + index, (unsigned char*)data + offset, count);
	return 0;
}
...
size_t c_vector_insert(c_vector* cVector, size_t index, void* data, size_t offset, size_t count) {
	...
	for (i = 1; i <= len; i++) {
		memcpy((unsigned char*)(cVector->items) + new_size - i, (unsigned char*)(cVector->items) + last_total - i, 1);
	}
	memcpy((unsigned char*)(cVector->items) + index, (unsigned char*)data + offset, count);
	return count;
}
...
void* c_vector_get(c_vector* cVector, size_t offset) {
	...
	return (unsigned char*)(cVector->items) + offset;
}
...
size_t c_vector_delete(c_vector* cVector, size_t offset, size_t count) {
	...
	memcpy((unsigned char*)(cVector->items) + offset, (unsigned char*)(cVector->items) + offset + count, cVector->total - count - offset);
...
size_t c_vector_read(void* p_buffer, size_t p_nb_bytes, c_vector* v) {
	...
	memcpy((unsigned char*)p_buffer, (unsigned char*)(v->items) + v->index, p_nb_bytes);
	v->index += p_nb_bytes;
	return p_nb_bytes;
}
