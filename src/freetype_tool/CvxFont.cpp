...
void cvx::putText(cv::Mat &img, const std::string &text, const cv::Point &pos, cvx::CvxFont &fontFace, int fontSize,
                  const cv::Scalar &color) {
    fontFace.setFontSize(fontSize);
#if defined(_MSC_VER)
    fontFace.putTextStr_(img, text, std::move(pos), color);
#else
    fontFace.putTextu32Str_(img, strToU32String(text), pos, color);
#endif
    fontFace.initFont_();
}

void cvx::putOneSymbol(cv::Mat &img, uint32_t symbol, cv::Point pos, cvx::CvxFont &fontFace, int fontSize,
                       const cv::Scalar &color) {
    fontFace.setFontSize(fontSize);
...
void cvx::putSymbols(cv::Mat &img, std::vector<uint32_t> &symbols, cv::Point pos, cvx::CvxFont &fontFace, int fontSize,
                     const cv::Scalar &color) {
    fontFace.setFontSize(fontSize);
...
void cvx::CvxFont::putText(cv::Mat &img, const std::string &data, const cv::Point &point, int fontSize) {
    cvx::putText(img, data, point, *this, fontSize, cv::Scalar(0, 0, 0));
}

void cvx::CvxFont::putText(cv::Mat &img, std::vector<uint32_t> &data, const cv::Point &point, int fontSize) {
    cvx::putSymbols(img, data, point, *this, fontSize, cv::Scalar(0, 0, 0));
}
