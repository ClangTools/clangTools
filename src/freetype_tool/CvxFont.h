...
/**
 * https://github.com/busyboxs/freetype_opencv
 */
#include <ft2build.h>
...
namespace cvx {
    ...
    public:
        ...
        explicit CvxFont(const cv::String &fontType = "./OpenDotMatrixFont.ttf");
        ...
        inline bool isLoad() const { return m_font != nullptr; }

        void putText(cv::Mat &, const std::string &, const cv::Point &, int fontSize = 14);

        void putText(cv::Mat &, std::vector<uint32_t> &, const cv::Point &, int fontSize = 14);
    ...
    private:
        ...
        friend void putText(cv::Mat &, const std::string &, const cv::Point &, cvx::CvxFont &, int, const cv::Scalar &);
    ...
    void putText(cv::Mat &img, const std::string &text, const cv::Point &pos, cvx::CvxFont &fontFace, int fontSize,
                 const cv::Scalar &color);

    void putSymbols(cv::Mat &img, std::vector<uint32_t> &symbols, cv::Point pos, cvx::CvxFont &fontFace, int fontSize,
                    const cv::Scalar &color);

    void putOneSymbol(cv::Mat &img, uint32_t symbol, cv::Point pos, cvx::CvxFont &fontFace, int fontSize,
                      const cv::Scalar &color);
