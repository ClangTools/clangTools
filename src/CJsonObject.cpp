...
cJSON_uint64 CJsonObject::toNumber() const {
    ...
    auto d = toString();
    return strtol(d.c_str(), nullptr, 10);
}
...
std::string CJsonObject::operator()(const std::string &strKey) const {
    ...
    if (pJsonStruct->type == cJSON_String) {
        ...
        if (pJsonStruct->sign == -1) {
            if ((cJSON_int64) pJsonStruct->valueint <= (cJSON_int64) INT_MAX &&
                (cJSON_int64) pJsonStruct->valueint >= (cJSON_int64) INT_MIN) {
