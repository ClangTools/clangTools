...
enum {
    LFS_CMP_EQ = 0, LFS_CMP_LT = 1, LFS_CMP_GT = 2, };
    ...
    while (true) {
        {
            // setup commit state
            struct lfs_commit commit = {
                    .block = dir->pair[1], .off = 0, .ptag = 0xffffffff, .crc = 0xffffffff, .begin = 0, .end =
...
static int lfs_dir_commit(lfs_t *lfs, lfs_mdir_t *dir,
    ...
    if (dir->erased || dir->count >= 0xff) {
        // try to commit
        struct lfs_commit commit = {
                .block = dir->pair[0], .off = dir->off, .ptag = dir->etag, .crc = 0xffffffff, .begin = dir->off, .end =
                lfs->cfg->block_size - 8,
        };
...
static int lfs_file_flush(lfs_t *lfs, lfs_file_t *file) {
    ...
    if (file->flags & LFS_F_WRITING) {
        ...
        if (!(file->flags & LFS_F_INLINE)) {
            ...
            lfs_file_t orig = {
                    .ctz.head = file->ctz.head, .ctz.size = file->ctz.size, .flags = LFS_O_RDONLY |
                                                                                     LFS_F_OPENED, .pos = file->pos, .cache = lfs->rcache,
            };
...
int lfs_format(lfs_t *lfs, const struct lfs_config *cfg) {
    ...
    int err = 0;
    {
        ...
        lfs_superblock_t superblock = {
                .version     = LFS_DISK_VERSION, .block_size  = lfs->cfg->block_size, .block_count = lfs->cfg->block_count, .name_max    = lfs->name_max, .file_max    = lfs->file_max, .attr_max    = lfs->attr_max,
        };
